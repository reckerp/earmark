...
export const addAudiobook = async (prevState: FormState, input: FormData) => {
  ...
  try {
    ...
    revalidatePath("/");
    return {
      message: `Audiobook ${mark.title} added successfully`,
      type: "success",
    };
  } catch (error) {
...
const refreshAccessToken = async (refreshToken: string) => {
  ...
  const authEncoded = Buffer.from(clientId + ":" + clientSecret).toString(
    "base64",
  );
